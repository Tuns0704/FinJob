...
namespace finjob_backend.Repository.IRepository
{
    ...
    {
        Task<PaginationResult<T>> GetAllAsync(Expression<Func<T, bool>>? filter = null, string? search = null, int pageSize = 0, int pageNumber = 1, params Expression<Func<T, object>>[]? includes);
