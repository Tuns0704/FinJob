				...
				<nav className="flex align-middle py-6 justify-between items-center navbar">
					...
					<ul className="list-none sm:flex hidden justify-center items-center flex-1">
						{navLinks.map((nav, index) => (
							<li
								key={nav.id}
								className={`px-5 py-2 rounded-3xl font-normal  cursor-pointer text-[16px]  hover:bg-lightOrange hover:text-primary ${
					...
					<div>
						{isAuthenticated ? (
							<div>
								<button className="bg-primary text-white px-5 py-2 rounded-3xl font-normal text-[16px] hover:bg-lightOrange hover:text-primary">
									{user.name}
								</button>
								<button
